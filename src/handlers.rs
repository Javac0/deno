...
type DenoOp = Future<Item = deno_buf, Error = DenoError>;
type OpResult = DenoResult<deno_buf>;
type Handler = fn(d: *const DenoC, base: &msg::Base) -> Box<DenoOp>;
...
pub extern "C" fn msg_from_js(d: *const DenoC, buf: deno_buf) {
  ...
  if base.sync() {
    ...
    //let buf = deno.rt.block_on(future).unwrap();
    ...
    let future = future.and_then(move |buf| {
...
fn handle_exit(_d: *const DenoC, base: &msg::Base) -> Box<DenoOp> {
  let msg = base.msg_as_exit().unwrap();
  std::process::exit(msg.code())
  // futures::future::ok::<deno_buf, DenoError>(null_buf())
}

fn handle_start(d: *const DenoC, base: &msg::Base) -> Box<DenoOp> {
  let deno = from_c(d);
...
// TODO add cmd_id as a parameter
fn create_msg(
...
fn ok_future(buf: deno_buf) -> Box<DenoOp> {
  Box::new(futures::future::ok(buf))
}

// Shout out to Earl Sweatshirt.
fn odd_future(err: DenoError) -> Box<DenoOp> {
  Box::new(futures::future::err(err))
}

// https://github.com/denoland/deno/blob/golang/os.go#L100-L154
fn handle_code_fetch(d: *const DenoC, base: &msg::Base) -> Box<DenoOp> {
...
fn handle_code_cache(d: *const DenoC, base: &msg::Base) -> Box<DenoOp> {
...
fn handle_set_env(d: *const DenoC, base: &msg::Base) -> Box<DenoOp> {
...
fn handle_env(d: *const DenoC, _base: &msg::Base) -> Box<DenoOp> {
  let deno = from_c(d);
  if !deno.flags.allow_env {
    return odd_future(permission_denied());
  }
...
fn handle_fetch_req(d: *const DenoC, base: &msg::Base) -> Box<DenoOp> {
...
fn handle_make_temp_dir(d: *const DenoC, base: &msg::Base) -> Box<DenoOp> {
...
fn handle_mkdir_sync(d: *const DenoC, base: &msg::Base) -> Box<DenoOp> {
...
fn handle_read_file_sync(_d: *const DenoC, base: &msg::Base) -> Box<DenoOp> {
...
fn handle_stat_sync(_d: *const DenoC, base: &msg::Base) -> Box<DenoOp> {
...
fn handle_write_file_sync(d: *const DenoC, base: &msg::Base) -> Box<DenoOp> {
...
fn handle_timer_start(d: *const DenoC, base: &msg::Base) -> Box<DenoOp> {
...
fn handle_timer_clear(d: *const DenoC, base: &msg::Base) -> Box<DenoOp> {
...
fn handle_rename_sync(d: *const DenoC, base: &msg::Base) -> Box<DenoOp> {
  let deno = from_c(d);
  if !deno.flags.allow_write {
    return Box::new(futures::future::err(permission_denied()));
  };
